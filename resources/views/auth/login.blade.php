...
<form method="post" action="{{ route('login') }}" class="panel">
    @csrf
    <x-field name="email" type="email" :label="__('auth.email')" :value="$email ?? ''" required autofocus autocomplete="email" inputmode="email" />
    <x-field name="password" type="password" :label="__('auth.password')" required autocomplete="current-password" />
