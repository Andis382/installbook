...
const service = computed(() => {
  if (props.status === 'REMOVED') return { tone: 'neutral' as const, text: t('pills.removed', { date: formatDate(props.removedOn) }) }
  ...
  if (props.serviceState === 'OVERDUE')
    text = -days > LONG_OVERDUE_DAYS ? t('pills.overdueSince', { month: formatMonth(props.nextServiceDue) }) : t('pills.overdue', { n: -days }, -days)
  else if (props.serviceState === 'DUE_SOON') text = days === 0 ? t('pills.dueToday') : t('pills.dueIn', { n: days }, days)
...
<template>
  <span class="pills">
    <UiBadge
      ...
      :size="size"
    >
      {{ warrantyActive ? $t('pills.warrantyUntil', { date: formatDate(warrantyUntil) }) : $t('pills.warrantyEnded', { date: formatDate(warrantyUntil) }) }}
    </UiBadge>
    <UiBadge
