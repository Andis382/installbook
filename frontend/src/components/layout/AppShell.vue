...
import { PhCaretDown, PhDotsThreeOutline, PhSignOut } from '@phosphor-icons/vue'
...
const links = computed(() => items.value.filter((i) => !i.cta))
const bandLinks = computed(() => links.value.filter((i) => !i.accountMenu))
const accountLinks = computed(() => links.value.filter((i) => i.accountMenu))
const cta = computed(() => items.value.find((i) => i.cta) ?? null)
const primary = computed(() => items.value.filter((i) => i.primary).slice(0, 3))
/** Phone bar: two tabs, the raised call to action, the rest of the tabs, then "More". */
const barStart = computed(() => primary.value.slice(0, 2))
const barEnd = computed(() => primary.value.slice(2))
...
.band::before {
  /* blueprint grid: fine lines every 24px, a stronger one every 96px */
  content: '';
  position: absolute;
  inset: 0;
  z-index: -1;
  ...
  mask-image: linear-gradient(180deg, #000 0%, rgb(0 0 0 / 0.6) 55%, transparent 100%);
}
.band::after {
  content: '';
  position: absolute;
...
.band__cta {
  ...
  border-radius: var(--radius-pill);
  color: var(--on-accent);
  background: linear-gradient(180deg, var(--accent-400), var(--accent-500) 60%, var(--accent-600));
  box-shadow:
...
.band__cta:hover {
  ...
  box-shadow:
    inset 0 1px 0 rgb(255 255 255 / 0.3),
    0 12px 26px -8px rgb(0 0 0 / 0.6);
}
.band__cta:focus-visible {
...
.tabbar__cta:active .tabbar__fab {
  transform: scale(0.96);
}
.tabbar__cta:focus-visible .tabbar__fab {
  box-shadow: 0 0 0 4px color-mix(in srgb, var(--accent-500) 40%, transparent);
}
