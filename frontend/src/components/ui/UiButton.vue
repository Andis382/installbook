...
const props = withDefaults(
  defineProps<{
    variant?: 'primary' | 'accent' | 'secondary' | 'ghost' | 'soft' | 'danger' | 'inverse'
    size?: 'sm' | 'md' | 'lg'
...
/**
 * Only the attributes the rendered element needs. Binding href="undefined" onto a RouterLink
 * would override the href it computes, leaving an <a> without href: not a link, not focusable.
 */
const bindings = computed(() => {
  if (props.to) return { to: props.to, 'aria-disabled': props.disabled ? 'true' : undefined }
  if (props.href) {
    return {
      href: props.href,
      target: props.target,
...
.btn--accent:hover:not(:disabled) {
  ...
  box-shadow:
    inset 0 1px 0 rgb(255 255 255 / 0.28),
    0 2px 4px rgb(16 24 40 / 0.14),
    0 12px 24px -8px color-mix(in srgb, var(--accent-600) 80%, transparent);
}
.btn--accent:focus-visible {
  box-shadow:
    inset 0 1px 0 rgb(255 255 255 / 0.28),
    var(--accent-shadow),
    0 0 0 4px color-mix(in srgb, var(--accent-500) 40%, transparent);
}
