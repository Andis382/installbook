...
import { PhBellRinging, PhBuildings, PhLinkSimple, PhTrash, PhUserCircle, PhUsersThree } from '@phosphor-icons/vue'
    ...
    <div class="grid-2">
      ...
      <UiCard :title="$t('settings.organization')" :icon="PhBuildings">
        <form class="stack" novalidate @submit.prevent="saveOrg">
          ...
          <UiField id="f-org-phone" :label="$t('settings.phone')" :hint="$t('settings.phoneHint')" :error="org.error('phone')">
            <template #default="{ id, invalid, describedby }">
      ...
      <UiCard class="span-all" :title="$t('settings.installer')" :subtitle="$t('settings.installerHint')" :icon="PhBellRinging">
        <form class="installer" novalidate @submit.prevent="saveInstaller">
          <UiFormErrors class="span-all" :errors="installer.errors.value" :message="installer.message.value" :trigger="installer.submitted.value" />
          <UiField id="f-defaultWarrantyMonths" :label="`${$t('settings.defaultWarranty')} (${$t('settings.monthsUnit')})`" :error="installer.error('defaultWarrantyMonths')">
            <UiStepper id="f-defaultWarrantyMonths" v-model="installer.data.defaultWarrantyMonths" :label="$t('settings.defaultWarranty')" :min="1" :max="120" :step="6" />
          </UiField>
          <UiField id="f-defaultServiceIntervalMonths" :label="`${$t('settings.defaultInterval')} (${$t('settings.monthsUnit')})`" :error="installer.error('defaultServiceIntervalMonths')">
            <UiStepper id="f-defaultServiceIntervalMonths" v-model="installer.data.defaultServiceIntervalMonths" :label="$t('settings.defaultInterval')" :min="1" :max="60" />
          </UiField>
          <UiField id="f-reminderLeadDays" :label="`${$t('settings.leadDays')} (${$t('settings.leadDaysUnit')})`" :hint="$t('settings.leadDaysHint')" :error="installer.error('reminderLeadDays')">
            <UiStepper id="f-reminderLeadDays" v-model="installer.data.reminderLeadDays" :label="$t('settings.leadDays')" :min="0" :max="90" :step="5" />
          </UiField>
          <UiField id="f-typicalServicePriceCents" :label="$t('settings.price')" :hint="$t('settings.priceHint')" :error="installer.error('typicalServicePriceCents')">
            <template #default="{ id, invalid, describedby }">
              <UiInput :id="id" v-model="installer.data.price" inputmode="decimal" prefix="€" :invalid="invalid" :describedby="describedby" :disabled="!isOwner" />
            </template>
          </UiField>
          <UiField id="f-publicPhone" :label="$t('settings.publicPhone')" :hint="$t('settings.publicPhoneHint')" :error="installer.error('publicPhone')" optional>
            <template #default="{ id, invalid, describedby }">
              <UiInput :id="id" v-model="installer.data.publicPhone" type="tel" inputmode="tel" :invalid="invalid" :describedby="describedby" :disabled="!isOwner" />
            </template>
          </UiField>
          <div v-if="isOwner" class="installer__save"><UiButton type="submit" :loading="installer.processing.value">{{ $t('common.save') }}</UiButton></div>
        </form>
      </UiCard>
